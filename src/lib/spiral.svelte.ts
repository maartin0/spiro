...
function scale([x, y]: [number, number], mul: number, scale: number): [number, number] {
	return [Math.round(((x / 4) * scale + 0.5) * mul), Math.round(((y / 4) * scale + 0.5) * mul)];
}
...
export function drawSpiral(
	...
	// Multiplier to scale each pixel coordinate into the canvas
	const multiplier = canvasDimensions.height;
	const scalar = spiral.ty === SpiralTypeEnum.Hypotrochoid ? 1.8 : 1;
	const end = spiral.props.streakLength * Math.PI * 2000;
	let startPixel = scale(computePosition(spiral, 0), multiplier, scalar);
	for (let t = 0; t <= end; t += RESOLUTION) {
		const pixel = scale(computePosition(spiral, t), multiplier, scalar);
		context.beginPath();
